...
public class ComplaintDAO {
	...
	public List<Complaint> getComplaintDetails(String oid, String cid,
			String mob) {
	...
	public static List<Order> getAllOrderDetails(String oid) {
		...
		try {
			...
			// Customer(rs.getString("name"),rs.getString("mobile"),rs.getString("email"));

			// Order ord = new
			// Order(rs.getInt("_id"),rs.getInt("amount"),rs.getString("date"),rs.getTime("time").toString(),customer);
			// order.add(ord);
			// }
		} catch (Exception e) {
	...
	public static List<Order> getOrderDetails(String name, String oid, String mob) {
		...
		try {
			...
			String query = "";
			con = DBConnection.getConnection();
			if (name != null && name != "" && oid != null && oid != ""
					&& mob != null && mob != "") {
				query = "select o._id,o.amount,DATE_FORMAT(o.time,'%Y-%m-%d') as `date`,DATE_FORMAT(o.time,'%H:%i:%s') as `time`,c.name as `name`,c.mobile as `mobile`,c.email as `email` from `order` as o,customer as c where c.user_id=o.cust_id And c.name like '%"
						+ name
						+ "%' And o._id='"
						+ oid
						+ "' And c.mobile='"
				...
				query = "select o._id,o.amount,DATE_FORMAT(o.time,'%Y-%m-%d') as `date`,DATE_FORMAT(o.time,'%H:%i:%s') as `time`,c.name as `name`,c.mobile as `mobile`,c.email as `email` from `order` as o,customer as c where c.user_id=o.cust_id And c.name like '%"
						+ name + "%' And o._id='" + oid + "' order by `date` ";
				...
				query = "select o._id,o.amount,DATE_FORMAT(o.time,'%Y-%m-%d') as `date`,DATE_FORMAT(o.time,'%H:%i:%s') as `time`,c.name as `name`,c.mobile as `mobile`,c.email as `email` from `order` as o,customer as c where c.user_id=o.cust_id And c.mobile='"
						+ mob + "' and o._id='" + oid + "' order by `date` ";
				...
				query = "select o._id,o.amount,DATE_FORMAT(o.time,'%Y-%m-%d') as `date`,DATE_FORMAT(o.time,'%H:%i:%s') as `time`,c.name as `name`,c.mobile as `mobile`,c.email as `email` from `order` as o,customer as c where c.user_id=o.cust_id And o._id='"
						+ oid + "' order by `date` ";
			...
			while (rs.next()) {
				...
				Order ord = new Order(rs.getInt("_id"), rs.getInt("amount"),
						rs.getString("date"), rs.getTime("time").toString(),
						customer);
				order.add(ord);
			}
		} catch (Exception e) {
